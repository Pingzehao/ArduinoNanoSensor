...
public class GraphView extends SurfaceView implements SurfaceHolder.Callback, Serializable
{
	...
	/** Flag for turning on temp offset fix **/
	private static final boolean OFFSET_TEMP = true;
	
	/** BitVoltage value to subtract from temp measurement calculated after taking first temp measurement **/
	private int tempVoltageOffset = 0;
	...
	private static final int TEMP_WINDOW_MAX = 246;
	private static final int TEMP_WINDOW_MIN = 62;
	...
	public void drawAxis(Canvas c)
	{
		...
		switch(mDisplayMode)
		{
			...
			case TEMPERATURE:
				c.drawLine(15, 5, 15, getHeight() - 5, mAxisPaint);
				c.drawLine(5, getHeight() - 15, getWidth() - 5, getHeight() - 15, mAxisPaint);
				c.drawText(df.format(calculateTemp(TEMP_WINDOW_MAX)) + "C", 20, 10, mAxisPaint);
				c.drawText(df.format(calculateTemp(TEMP_WINDOW_MIN)) + "C", 20, getHeight() - 5, mAxisPaint);
	...
	public void drawTempData(Canvas c, ArrayList<Data> dataToDraw)
	{
		float height = getHeight() - 15;
		float width = (float) (getWidth() / 1.5) - 5;
		float startX = 0;
	...
	public void drawData(Canvas c)
	{
		...
		switch(mDisplayMode)
		{
			...
			case TEMPERATURE:
				if(AVERAGE_DATA)
					drawTempData(c, mAveragedData.get(TEMPERATURE));
				else
					drawTempData(c, mDataPoints.get(TEMPERATURE));
				break;
			case GPS:
				drawGPSData(c);
				break;
	...
	public void addData(int[] values, Calendar timeStarted)
	{
		...
		if(OFFSET_TEMP)
		{
			if(mDataPoints.get(TEMPERATURE).size() == 1)
			{
				tempVoltageOffset = mDataPoints.get(TEMPERATURE).get(0).mVoltage - 150;
				mDataPoints.get(TEMPERATURE).get(0).mVoltage -= tempVoltageOffset;
			}
			values[TEMPERATURE] -= tempVoltageOffset;
		}
		for(int i = 0; i < ANALOG_PINS; ++i)
		{
