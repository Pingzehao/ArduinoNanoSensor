...
public class ByteQueue
{
	...
	public int[] writePolling(byte[] buffer, int offset, int length) throws InterruptedException
	{
		if(length + offset > buffer.length)
		{
			throw new IllegalArgumentException("length + offset > buffer.length");
				...
				if(bytesToCopy > 1)
				{
					...
					if(s.charAt(i) == ';')
					{
						...
						for(int j = 0; j < ANALOG_PINS; ++j)
						{
							...
							{
								Log.e("ArduinoGUI", "ByteQueue::writePolling:" + "ErrorParsingStringToInt: " + voltage);
								clearBuffer();
							}
						}
						// tail = 0;
	...
	public byte[] write(byte[] buffer, int offset, int length) throws InterruptedException
	{
		if(length + offset > buffer.length)
		{
			throw new IllegalArgumentException("length + offset > buffer.length");
