...
public class GraphPanel extends JPanel
{
	...
	private static final int GRAPH_RESISTANCE_AXIS_TICK_ROUNDOFF = 5; 
	...
	private void processData(ArrayList<Double> times,
		...
		mResistanceAxis *= AXIS_RESISTANCE_MARGIN;
		boolean negativeResistance = false;
		if (mResistanceAxis < 0) {
			negativeResistance = true;
			mResistanceAxis = -mResistanceAxis;
		}
		boolean smallResistance = false;
		if (mResistanceAxis < 5) {
			mResistanceAxis *= 1000;
	...
	private void drawResistanceAxis(Graphics g) {
		...
		double resistanceSpacing = mResistanceAxis / (GRAPH_RESISTANCE_AXIS_TICKS);
		...
		for (int i = 0; i < GRAPH_RESISTANCE_AXIS_TICKS; ++i) {
			...
			if (i % 2 == 0) {
				stopX = startX + GRAPH_MAJOR_AXIS_TICK_SIZE;
				double resistance = resistanceSpacing * i;
