...
public class NanoSensorGrapher extends WindowAdapter
{
	...
	private static final String ERROR_FILE_READ = "Error: Error parsing data file";
	...
	private static void processFiles() {
		...
		for (int i = 0; i < selectedFiles.size(); ++i) {
			...
			Thread processingThread = new Thread(new Runnable()
			{
				...
				public void run() {
					try {
						generateGraph(file, finalGasConcentration);
					} catch (IOException e) {
	...
	public static void generateGraph(File file, String concentration) throws IOException {
		...
		GraphPanel graphPanel = 
				new GraphPanel(timeData, resistanceData, concentration, dataFileName);
