...
public class GraphView extends SurfaceView implements SurfaceHolder.Callback
{
	...
	public static final int MQ2 = 16;
	...
	/** Location listener for geting GPS coordinates **/
	private LocationManager mLocManager = null;
	private LocationListener mLocListener;
	
	/** Variables for calculating the commercial sensor readings **/
	private double mMQ2InitialResistance = -1;
	private static final double MQ2_DIVIDER_RESISTANCE = 10.0;
	
	/** Array to hold data for sending **/
	private double[] mNetworkData = new double[ANALOG_INPUTS];
	
	/** Time left on baseline */
	...
	private boolean[] mHiddenPins = new boolean[ANALOG_INPUTS];

	private static int mDisplayMode = CHIP;

	// Window scaling values
	private boolean mAutoScaling = false;
	private int[] mMinValues = new int[ANALOG_INPUTS];
	private int[] mMaxValues = new int[ANALOG_INPUTS];
	...
	public void initializeValues()
	{
		...
		/** Pin Paints */
		mPinPaints.get(0).setColor(Color.rgb(255, 0, 0));
		mPinPaints.get(1).setColor(Color.rgb(0, 255, 0));
		mPinPaints.get(2).setColor(Color.rgb(0, 0, 255));
		mPinPaints.get(3).setColor(Color.rgb(255, 255, 0));
		mPinPaints.get(4).setColor(Color.rgb(255, 0, 255));
		mPinPaints.get(5).setColor(Color.rgb(0, 255, 255));
		mPinPaints.get(6).setColor(Color.rgb(255, 128, 0));
		mPinPaints.get(7).setColor(Color.rgb(255, 0, 128));
		mPinPaints.get(8).setColor(Color.rgb(128, 255, 0));
		mPinPaints.get(9).setColor(Color.rgb(0, 255, 128));
		mPinPaints.get(10).setColor(Color.rgb(128, 0, 255));
		mPinPaints.get(11).setColor(Color.rgb(0, 128, 255));
		mPinPaints.get(12).setColor(Color.rgb(128, 0, 0));
		mPinPaints.get(13).setColor(Color.rgb(0, 128, 0));
		mPinPaints.get(14).setColor(Color.rgb(0, 0, 128));
		mPinPaints.get(15).setColor(Color.rgb(128, 128, 128));
		mPinPaints.get(MQ2).setColor(Color.BLACK);
		mPinPaints.get(HUMIDITY).setColor(Color.BLACK);
	...
	public void drawAxis(Canvas c)
	{
		...
		switch(mDisplayMode)
		{
			...
			case GPS:
				...
				break;
			case TIMER:
				Paint graphPaint = new Paint(mAxisPaint);
				float startX = 5;
				float startY = getHeight() / 2;
				String timeLeft = "" + mTimeLeft / 1000.0 + "s";

				float oldTextSize = graphPaint.getTextSize();
				float oldTextScale = graphPaint.getTextScaleX();
				float targetHeight = getHeight() * 0.25f;
				float targetWidth = getWidth() * 0.8f;
				setTextSize(timeLeft, graphPaint, 36, targetHeight);
				setTextScale(timeLeft, graphPaint, 1, targetWidth);
				
				float textHeight = getTextHeight(timeLeft, graphPaint);
				float textWidth = getTextWidth(timeLeft, graphPaint);
				
				startX = (getWidth() - textWidth) / 2;
				startY = (getHeight() + textHeight) / 2;
				
				c.drawText(timeLeft, startX, startY, graphPaint);
	...
	{
		double voltage = REFERENCE_VOLTAGE * (bitVoltage / 1023.0);
		double tempCelcius = (voltage - 0.75) * 100 + 25;
		return tempCelcius;
	}
	...
	public void setCountdown(long timeLeft)
	{
		mTimeLeft = timeLeft;
	}
}
