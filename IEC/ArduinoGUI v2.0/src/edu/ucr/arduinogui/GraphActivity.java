...
public class GraphActivity extends Activity
{
	...
	/** Constants used for the pins of the ADC on the Arduino */
	public static final int CHIP = 4;
	public static final int MQ2 = 5;
	...
	/** Boolean array indicating which pins are hidden and to not draw */
	private static boolean[] pinsChecked = new boolean[ANALOG_PINS];

	/** Flag indicating whether or not autoscale shoudl be used */
	public static boolean autoScale = false;

	/** Used to change the menu icon when connected or disconnected */
				...
				while(true)
				{
					if(mSerialService.isPolling())
					{
								...
								&& dataToAdd[7] != -1)
						{
							...
							/**
							 * Resets all the values to -1 so the same
							 * value doesn't get added twice
							 */
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		if(event.getAction() == MotionEvent.ACTION_UP)
		{
			...
			{
				case CHIP:
					graphView.setDisplayMode(MQ2);
					Toast.makeText(getApplicationContext(),
							"MQ2 Sensor Data", Toast.LENGTH_SHORT)
							.show();
					break;
				case MQ2:
					graphView.setDisplayMode(HUMIDITY);
					Toast.makeText(getApplicationContext(),
							"Humidity Sensor Data", Toast.LENGTH_SHORT)
							.show();
					break;
				case HUMIDITY:
					...
					break;
				case TEMPERATURE:
					graphView.setDisplayMode(GPS);
					Toast.makeText(getApplicationContext(), "GPS Data",
							Toast.LENGTH_SHORT).show();
					break;
				case GPS:
					graphView.setDisplayMode(CHIP);
					Toast.makeText(getApplicationContext(),
							"Chip Sensor Data", Toast.LENGTH_SHORT)
							.show();
					break;
			}
			return true;
		...
		public void handleMessage(Message msg)
		{
			switch(msg.what)
			{
				case MESSAGE_STATE_CHANGE:
					switch(msg.arg1)
					{
						case BluetoothSerialService.STATE_CONNECTED:
							...
							TabContainer.getmTitle().setText(
									R.string.title_connected_to);
							TabContainer.getmTitle().append(
					...
					break;
				/*
				 * case MESSAGE_WRITE: if (mLocalEcho) { byte[] writeBuf =
				 * (byte[]) msg.obj; mEmulatorView.write(writeBuf,
				 * msg.arg1); }
				 * 
				 * break;
				 */
				case MESSAGE_READ:
					...
					// TextView t = (TextView)
					// findViewById(R.id.outputDisplay);
	...
	public static void startPolling(boolean[] pinsChecked, String output,
			int pollingRate, String email, double percentThreshold)
	{
		mSerialService.stopPolling();
		mSerialService.clearBuffer();
		graphView.reset();
		timeStarted = Calendar.getInstance();
		graphView.setAutoEmail(email);
		graphView.setThreshold(percentThreshold);
	...
	/** backbutton from killing the app */
