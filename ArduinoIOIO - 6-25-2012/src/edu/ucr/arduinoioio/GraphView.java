...
public class GraphView extends SurfaceView implements SurfaceHolder.Callback, Serializable
{
	...
	public class Data
	{
		...
		private void calculateResistance(int mode, int pin)
		{
			...
			{
				mResistance = ((double) mVoltage / 1023 * INPUT_VOLTAGE * mDividerResistance[pin]) / ((1023 - mVoltage) * INPUT_VOLTAGE);
			}
	...
	public void createFiles(Calendar timeStarted)
	{
		try
		{
			...
			mNetworkFile = new File(ROOT + "/ArduinoIOIO/", "networkData");
	...
	public void drawAxis(Canvas c)
	{
		...
		switch(mDisplayMode)
		{
			case CHIP:
				...
				c.drawText("" + df.format(mWindowMax / 1023 * MAX_RESISTANCE / 1000) + "kOhms", 20, 10, mAxisPaint);
				c.drawText("" + df.format(mWindowMin / 1023 * MAX_RESISTANCE / 1000) + "kOhms", 20, getHeight() - 5, mAxisPaint);
		...
		if(AVERAGE_DATA)
		{
			lastIndex = mAveragedData.get(7).size() - 1;
			if(lastIndex < 0)
				return;
		...
		{
			lastIndex = mDataPoints.get(7).size() - 1;
			if(lastIndex < 0)
				return;
		}
	...
	{
		double voltage = REFERENCE_VOLTAGE * (bitVoltage / 1023.0);
		double MQ2Resistance = (REFERENCE_VOLTAGE / voltage - 1) * 10;
		double ppm = -45.87 * Math.log(MQ2Resistance) + 147.12;
	...
	public void writeChipData(ArrayList<ArrayList<Data>> dataToWrite)
	{
		...
		for(int i = 0; i < CHIP; ++i)
		{
			...
			{
				Log.e(LOG, "Error writing Chip Data to File ");
				e.printStackTrace();
			}
		}
	}
	...
	public void writeMQ2Data(ArrayList<Data> dataToWrite)
	{
		...
		{
			Log.e(LOG, "Error writing MQ2 Data to File");
			e.printStackTrace();
		}
	}
	...
	public void writeHumidityData(ArrayList<Data> dataToWrite, ArrayList<Data> temperatureData)
	{
		...
		{
			Log.e(LOG, "Error writing Humidity Data to File ");
			e.printStackTrace();
		}
	}
	...
	public void addData(int[] values, int pollRate, Calendar timeStarted)
	{
		...
		if(AVERAGE_DATA && mDataPoints.get(7).size() > POINTS_TO_AVERAGE)
		{
		...
		else
		{
			...
			writeHumidityData(mDataPoints.get(HUMIDITY), mAveragedData.get(TEMPERATURE));
	...
	public void setDividerResistances(int[] dividerResistances)
	{
		for(int i = 0; i < CHIP; ++i)
		{
			mDividerResistance[i] = dividerResistances[i] * 100000 / 255;
		}
	}
