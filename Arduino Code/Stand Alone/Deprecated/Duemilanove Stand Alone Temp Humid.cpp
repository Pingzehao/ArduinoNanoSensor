	...
	if(deviceStatus == STOPPED)
	{
		if(LCD.getTouchActivity() == 2)
		{
			...
			if(startButton.isClicked(x, y))
			{
				...
				calibratePot();
				LCD.clear();
				LCD.appendString2File("Data", "\nDate\tTime\tLongitude\tLatitude\tR0\tR1\tR2\tR3\tTemp\tRH%\n");
				x = -1;
				y = -1;
			}
		}
...
void readChip()
{
	...
	for(int i = 0; i < CHIP_PINS; ++i)
	{
		...
		if(deviceStatus == CHIP_VIEW)
		{
			...
			long rectTop = BAR_MID;
			long rectBot = BAR_MID;
			long rectDelta = BAR_MID * percentDelta / PERCENT_SCALE;
			
			if(rectDelta > BAR_MID)
				rectDelta = BAR_MID;
			else if(rectDelta < -BAR_MID)
				rectDelta = -BAR_MID;
			...
			// LCD.rectangle(rectLeft, BAR_MID, rectRight, BAR_MID, WHITE);
...
String doubleToString(double d, int precision)
{
	...
	int decimalNumber = (d - wholeNumber) * decimalPlaces;
