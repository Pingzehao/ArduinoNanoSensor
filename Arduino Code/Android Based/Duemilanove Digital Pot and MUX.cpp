...
#define MUX_SELECT1 2
#define MUX_SELECT2 3
#define MUX_SELECT3 4
#define MUX_SELECT4 5
...
int val2 = 0;
int state = TYPE;
int rsValues[CHIP_PINS];
boolean rsState[CHIP_PINS];
int rsRate;
int bitDividerResistance[16];
		...
		if(inByte != ' ') 
		{
			...
			case RATE:
				...
				else if(inByte == EOA)
				{
					...
					for(int i = 0; i < 5; ++i)
					{
						bitDividerResistance[i] = matchResistance(0, 255, 0);
						Serial.print(bitDividerResistance[i]);
						Serial.print('.');
					}
					for(int i = 0; i < 3; ++i)
					{
