...
uint8_t OLED::DrawRectangle(uint8_t x, uint8_t y, uint8_t x2, uint8_t y2, unsigned int color)
{
...
uint8_t OLED::DrawText(uint8_t column, uint8_t row, uint8_t font_size, char mychar, unsigned int color)
{
	char mytext[16];
...
uint8_t OLED::DrawText(uint8_t column, uint8_t row, uint8_t font_size, int myint, unsigned int color)
{
	char mytext[16];
...
uint8_t OLED::DrawText(uint8_t column, uint8_t row, uint8_t font_size, long mylong, unsigned int color)
{
	char mytext[16];
...
uint8_t OLED::DrawText(uint8_t column, uint8_t row, uint8_t font_size, double mydouble, unsigned int color)
{	
	char mytext[16];
...
uint8_t OLED::SetSDAddress(uint8_t bit1, uint8_t bit2, uint8_t bit3, uint8_t bit4)
{
	Serial.write(OLED_EXT_CMD);
	Serial.write(OLED_SET_MEM_ADDRESS);
	Serial.print((byte) bit1);
	// bit2 += 0x10;
	Serial.print((byte) bit2);
	Serial.print((byte) bit3);
	Serial.print((byte) bit4);
	GetResponse();
}

void OLED::writeSDByte(char c)
{
	Serial.write(OLED_EXT_CMD);
	Serial.write(OLED_WRITE_BYTE);
	Serial.print((byte) c);
	GetResponse();
}

void OLED::writeSDString(String s)
{
	for(int i = 0; i < s.length(); ++i)
	{
		writeSDByte(s.charAt(i));
	}
}
