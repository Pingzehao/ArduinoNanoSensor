...
public class GraphView extends SurfaceView implements SurfaceHolder.Callback, Serializable 
{
	...
	private static final double TEMP_WINDOW_MAX = 702.0;
	private static final double TEMP_WINDOW_MIN = 538.0;
	...
	public class Data
	{
		...
		private void calculateResistance(int mode, int pin)
		{
			...
			{
				mResistance = ((double) mVoltage / 1023 * INPUT_VOLTAGE * mDividerResistance[pin]) / 
	...
	private static final double TEMP_MIN = -10.048;
	private static final double TEMP_MAX = 70.109;
	private static final double TEMP_RANGE = TEMP_MAX - TEMP_MIN;
	...
	private int[] mDividerResistance = new int[CHIP];
	...
	public void createFiles(Calendar timeStarted)
	{
		try
		{
			...
			if(ROOT.canWrite())
			{
				...
				{
					//TODO: Change header to include GPS data and to show actual time also humidity?
	...
	{
		//TODO Draw the axis only on the left half for Chip data
		...
		switch(mDisplayMode)
		{
			case CHIP:
				...
				c.drawLine(5, getHeight() - 15, getWidth() / 2 - 5, getHeight() - 15, 
						axisPaint);
				...
				c.drawLine(getWidth() / 2, 5, getWidth() / 2, getHeight() - 15, axisPaint);	
				// Horizontal Bar, Length = (getWidth() - 5) - getWidth() / 2
				c.drawLine(getWidth() / 2, (getHeight() - 15) / 2 + 5, getWidth() - 5, (getHeight() - 15) / 2 + 5, axisPaint); 
				c.drawText("200%", (float) (getWidth() / 2), 10, axisPaint);
				c.drawText("-200%", (float) (getWidth() / 2), getHeight() - 5, axisPaint);
				break;
			case HUMIDITY:
				c.drawLine(15, 5, 15, getHeight() - 5, axisPaint);
			...
			case TEMPERATURE:
				...
				c.drawLine(5, getHeight() - 15, getWidth() - 5, getHeight() - 15, 
						axisPaint);
				c.drawText(df.format(TEMP_WINDOW_MAX / 1023 * 5 * 100 - 273) + "C", 20, 10, axisPaint);
				c.drawText(df.format(TEMP_WINDOW_MIN / 1023 * 5 * 100 - 273) + "C", 20, getHeight() - 5, axisPaint);
				break;
		}
		...
		if(AVERAGE_DATA)
		{
			...
			c.drawText("" + df.format(averagedData.get(0).get(lastIndex).mTime / 2) + "m" , 
					(float) ((getWidth() - 50) / 4.1), getHeight() - 5, axisPaint);
			c.drawText("" + df.format(averagedData.get(0).get(lastIndex).mTime) + "m" , 
					(float) ((getWidth() - 50) / 2.1), getHeight() - 5, axisPaint);
		}
		else
		{
			...
			switch(mDisplayMode)
			{
				...
				default:
					c.drawText("" + df.format(dataPoints.get(0).get(lastIndex).mTime / 2) + "m" , 
							(float) ((getWidth() - 50) / 2), getHeight() - 5, axisPaint);
					c.drawText("" + df.format(dataPoints.get(0).get(lastIndex).mTime) + "m" , 
							(float) ((getWidth() - 50)), getHeight() - 5, axisPaint);
			}
	...
	{
		//TODO Make the graph only draw on the left half, right half will be the delta bars
		...
		for(int i = 0; i < CHIP; ++i)
		{
			if(!mHiddenPins[i])
			{
				...
				//Draw Delta Graph
				double currentResistance = (double) (dataToDraw.get(i).get(dataToDraw.get(i).size() - 1)
						.mBitResistance);
				double initialResistance = (double) (dataToDraw.get(i).get(0).mBitResistance);
				if(currentResistance > 0)
				{
					//TODO Fix it so the graph is the same for 3k->6k or 6k->3k
					...
					c.drawRect(rectLeft, rectTop, rectRight, rectBot, pinPaints.get(i));
					
					//Draw Delta Graph Label
					float labelY = (float) (rectTop * 0.95);
					c.drawText(df.format(percentDelta * 100) + "%", rectLeft, labelY, pinPaints.get(i));
				}
			}
		}
		...
		for(int i = 0; i < CHIP; ++i)
		{
			if(!mHiddenPins[i])
			{
				...
				c.drawText(df.format(dataToDraw.get(i)
						.get(lastIndex).mResistance) + "kOhms", (float) (startX / 2.2), startY, pinPaints.get(i));
			}	
		}
	}
	...
	public void drawTempData(Canvas c, ArrayList<Data> dataToDraw, double temp)
	{
		...
		for(int i = 0; i < dataToDraw.size() - 2; ++i)
		{
			...
			double voltage = dataToDraw.get(i).mVoltage / 1023.0 * REFERENCE_VOLTAGE;
			double prevTemp = (voltage - 2.98) * 100 + 25;
			startY = (float) (height - (prevTemp - TEMP_MIN) / TEMP_RANGE * height);
			voltage = dataToDraw.get(i + 1).mVoltage / 1023.0 * REFERENCE_VOLTAGE;
			double currTemp = (voltage - 2.98) * 100 + 25;
			stopY = (float) (height - (currTemp - TEMP_MIN) / TEMP_RANGE * height);
		...
		startY = (float) (height - (dataToDraw.get(dataToDraw.size() - 1)
				.mVoltage - TEMP_WINDOW_MIN) * tempScale);
	...
	public void drawData(Canvas c)
	{
		...
		if(AVERAGE_DATA)
		{
			...
			if(lastIndex < 0)
				return;
			double voltage = averagedData.get(TEMPERATURE).get(lastIndex).mVoltage / 1023.0 * REFERENCE_VOLTAGE;
			tempCelcius = (voltage - 2.98) * 100 + 25;
		}
		else
		{
			...
			if(lastIndex < 0)
				return;
			double voltage = dataPoints.get(TEMPERATURE).get(lastIndex).mVoltage / 1023.0 * REFERENCE_VOLTAGE;
			tempCelcius = (voltage - 2.98) * 100 + 25;
		}
	...
	public void addData(int[] values, Calendar timeStarted)
	{
		...
		for(int i = 0; i < ANALOG_PINS; ++i)
		{
			if(i < TEMPERATURE)
				dataPoints.get(i).add(new Data(values[i], 
						dataPoints.get(i).size() * mPollingRate / MS_PER_MIN, i));
			else
				dataPoints.get(i).add(new Data(values[i] - TEMP_OFFSET, 
						dataPoints.get(i).size() * mPollingRate / MS_PER_MIN, i));
			if(AVERAGE_DATA)
			{
				if(dataPoints.get(i).size() > POINTS_TO_AVERAGE)
		...
		if(AVERAGE_DATA)
		{
			if(dataPoints.get(7).size() > POINTS_TO_AVERAGE)
			{
