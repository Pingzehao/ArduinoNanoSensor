...
{
	private static final String LOG = "ArudinoIOIO";
	...
	private static Thread mNetworkThread = new Thread(new Runnable()
	{
		@Override
		public void run()
		{
			while(true)
			{
				if(mClient != null)
				{
					while(mClient.isConnected() && mIsPolling && !mNetworkData.isEmpty())
					{
	...
	//TODO Change to getting the ip and port from the options activity?
//	public void sendNetworkData(int[] values)
//	{
//		Datagram toSend = new Datagram("client" + mClient.getPort(), values[0], values[1], values[2], values[3], values[4], values[5], values[6], mGraphView.getLatitude(), mGraphView.getLongitude());
//		int result = mClient.send(toSend);
//		if(result == -1)
//		{
//			Log.e(LOG,"edu.ucr.arduinoioio::GraphView.class::sendNetworkData(int[] values) - Error sending data");
//		}
//	}
		...
		public void addData(final int[] bitVoltages)
		{
			runOnUiThread(new Runnable()
			{
				...
				public void run()
				{
					...
					Datagram d = new Datagram("client" + mClient.getPort(), bitVoltages[0], bitVoltages[1], bitVoltages[2], bitVoltages[3], bitVoltages[4], bitVoltages[5], bitVoltages[6], mGraphView.getLatitude(), mGraphView.getLongitude());
					mNetworkData.add(d);
				}
			});
		}
...
//		{
//			Datagram d = new Datagram("init", dividerResistances[0], dividerResistances[1], dividerResistances[2], dividerResistances[3], mPollingRate, 0, 0, 0, 0);
//			mClient.send(d);
//		}
